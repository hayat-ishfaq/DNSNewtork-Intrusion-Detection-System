...
// DNS rule engine instance
static DNSRuleEngine dns_rule_engine;
...
PacketCapture::PacketCapture() 
    ...
    stats_.reset();
    // Load DNS rules at startup (hardcoded path for demo)
    dns_rule_engine.loadRules("rules/protocols/dns.rules");
}
...
void PacketCapture::processPacket(const struct pcap_pkthdr* packet_header,
    ...
    stats_.dns_packets++;
    // Apply DNS rules
    auto alerts = dns_rule_engine.match(*dns_packet);
    for (const auto& alert : alerts) {
        std::cout << "[ALERT] " << alert << " | src: " << src_ip << " | qname: ";
        if (!dns_packet->questions.empty())
            std::cout << dns_packet->questions[0].qname;
        std::cout << std::endl;
    }

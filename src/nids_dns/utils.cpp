...
namespace NetworkUtils {
    ...
    std::string formatIPv6Address(const uint8_t* ip_addr) {
        char buffer[46]; // Max IPv6 string length
#ifdef _WIN32
        // Windows: use inet_ntop from ws2tcpip.h
        if (inet_ntop(AF_INET6, ip_addr, buffer, sizeof(buffer)) == nullptr) {
            return std::string();
        }
#else
        if (inet_ntop(AF_INET6, ip_addr, buffer, sizeof(buffer)) == nullptr) {
    ...
    bool parseIPv4Address(const std::string& ip_str, uint32_t& ip_addr) {
        ...
        if (InetPtonA(AF_INET, ip_str.c_str(), &addr) == 1) {
            ip_addr = addr.s_addr;
            return true;
        }
#else
